        ...
        <section id="enterprise" className="flex flex-col">
            ...
            >
                <div className="relative mt-4 flex flex-col gap-12">
                    ...
                    {roles.map((role) => (
                        <div key={role.key} className="grid items-start gap-4 md:grid-cols-[180px_1fr] md:gap-10">
                            <div className="relative pl-6 md:pl-0 md:text-right">
                                ...
                                {(() => {
                                    const [dateLine, locationLine] = splitMeta(t(`${role.key}-meta`));
                                    return (
                            ...
                            <div className="flex flex-col gap-3">
                                <div className="flex flex-col gap-2">
                                    <h3 className="text-lg font-semibold text-slate-300">{t(`${role.key}-title`)}</h3>
                                </div>
                                <ul className="mt-2 space-y-2 text-sm text-slate-200">
                                    {role.bullets.map((bullet) => (
                                        <li key={`${role.key}-bullet-${bullet}`} className="flex gap-3 leading-relaxed">
                                            <span className="mt-2 inline-block h-2 w-2 rounded-full bg-gradient-to-r from-amber-200 to-sky-300"></span>
                                            <span>{t(`${role.key}-bullet-${bullet}`)}</span>
                                        </li>
                                    ))}
                                </ul>
                                <div className="flex flex-wrap gap-2">
                                    {getTags(role.tagsKey).map((tag: string) => (
                                        <span
                                            key={`${role.key}-tag-${tag}`}
